...
from sagemaker_training.beta.framework import content_types, encoders, env, transformer, worker
import test
...
def test_transformer_implementation():
    ...
    with worker.Worker(
        transform_fn=transform.transform, module_name="fake_ml_model"
        ...
        np.testing.assert_array_almost_equal(response_data, np.asarray([36.0, 81.0, 1764.0]))
...
def post(client, payload, content_type, accept):
    return client.post(
        ...
        data=encoders.encode(payload, content_type),
