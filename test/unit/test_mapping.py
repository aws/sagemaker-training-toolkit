...
import sagemaker_training.beta.framework as framework
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"nested": ["1", ["2", "3", [["6"]]]]}, {"SM_NESTED": '["1",["2","3",[["6"]]]]'}),
        (
            {"channel_dirs": {"eval": "bar", "train": "foo"}, "map": {"a": [1, 3, 4]}},
            {"SM_CHANNEL_DIRS": '{"eval":"bar","train":"foo"}', "SM_MAP": '{"a":[1,3,4]}'},
        ),
        ({"truthy": True, "falsy": False}, {"SM_FALSY": "false", "SM_TRUTHY": "true"}),
    ],
)
...
def test_env_vars_round_trip():
    ...
    resource_config = {"current_host": "algo-1", "hosts": ["algo-1", "algo-2", "algo-3"]}
    ...
    assert (
        env_vars["SM_INPUT_DATA_CONFIG"] == '{"train":{"ContentType":"trainingContentType",'
