...
@patch("sagemaker_training_toolkit._env._read_json", lambda x: {"a": 1})
@patch("json.loads")
...
def create_training_env():
    with patch(
        "sagemaker_training_toolkit._env.read_resource_config", lambda: RESOURCE_CONFIG
    ), patch(
        "sagemaker_training_toolkit._env.read_input_data_config", lambda: INPUT_DATA_CONFIG
    ), patch(
        "sagemaker_training_toolkit._env.read_hyperparameters", lambda: ALL_HYPERPARAMETERS
    ), patch(
        "sagemaker_training_toolkit._env.num_cpus", lambda: 8
    ), patch(
        "sagemaker_training_toolkit._env.num_gpus", lambda: 4
    ):
        session_mock = Mock()
        ...
        yield sagemaker_training_toolkit.training_env()
...
def create_serving_env():
    with patch("sagemaker_training_toolkit._env.num_cpus", lambda: 8), patch(
        "sagemaker_training_toolkit._env.num_gpus", lambda: 4
    ):
        old_environ = os.environ.copy()
...
def test_create_training_env_without_training_files_and_directories_should_not_fail():
    training_env = sagemaker_training_toolkit.training_env()
...
@patch("sagemaker_training_toolkit._env.num_cpus", lambda: 8)
@patch("sagemaker_training_toolkit._env.num_gpus", lambda: 4)
